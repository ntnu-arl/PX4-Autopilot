...
Eigen::MatrixXf openDataUnconstrained(std::string fileToOpen)
{
  ...
  return Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(matrixEntries.data(), matrixRowNumber, int(matrixEntries.size() / matrixRowNumber));
}
...
NeuralControlUnconstrained::NeuralControlUnconstrained(int n_motors)
{
  ...
  try
  {
    ...
    std::string path = "/home/paran/Documents/welf_hovers/Torch_to_Eigen/save_model/model_files/etor_task_b/";
